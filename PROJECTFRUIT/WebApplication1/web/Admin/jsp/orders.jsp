...
<!--            <div class="box-header">
            ...
            <form action="<%=request.getContextPath()%>/adminproController/updateOrder.htm" menthod="POST">
            <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                    <thead>
                            <tr>
                                    ...
                                    <th>List Product</th>
                                    <th>Order Date</th>
                                    <th colspan="3" style="text-align: center">Order Status</th>
                                    
                            </tr>
                    </thead>

                    <tbody>
                        <c:forEach items="${listOrder}" var="lpro">
                        <tr>
                            <td>${lpro.orderId}</td>
                            <td>${lpro.customerName}</td>
                            <td>${lpro.phone}</td>
                            <td>${lpro.amount}</td>
                            <td>${lpro.quantity}</td>
                            <td>${lpro.addresss}</td>
                            <td>${lpro.listProduct}</td>
                            <td>${lpro.orderDate}</td>
                            <c:if test = "${lpro.orderStatus == '1'}">
                                <td colspan="2" style="text-align: center">
                                    <a class="button remove-item"  href="<%=request.getContextPath()%>/adminproController/updateOrder.htm?orderId=${lpro.orderId}&statusName=xtd" onclick="return confirm('Bạn chắc chắn muốn xác thực đơn?')"><span><span>Xác thực đơn</span></span></a>
                                </td>
                                <td>
                                    <a class="button remove-item"  href="<%=request.getContextPath()%>/adminproController/updateOrder.htm?orderId=${lpro.orderId}&statusName=huydon" onclick="return confirm('Bạn chắc chắn muốn huỷ đơn?')"><span><span>Huỷ đơn</span></span></a>
                                </td>
                            </c:if>
                            <c:if test = "${lpro.orderStatus == '2'}">
                                <td>
                                    <span>Đang vận chuyển ...</span>
                                </td>
                                <td>
                                    <a class="button remove-item"  href="<%=request.getContextPath()%>/adminproController/updateOrder.htm?orderId=${lpro.orderId}&statusName=dtt" onclick="return confirm('Bạn chắc chắn khách hàng đã thanh toán?')"><span><span>Đã thanh toán</span></span></a>
                                </td>
                                <td>
                                    <a class="button remove-item"  href="<%=request.getContextPath()%>/adminproController/updateOrder.htm?orderId=${lpro.orderId}&statusName=huydon" onclick="return confirm('Bạn chắc chắn muốn xác thực đơn?')"><span><span>Huỷ đơn</span></span></a>
                                </td>
                            </c:if>
                            <c:if test = "${lpro.orderStatus == '3'}">
                                <td colspan="3" style="text-align: center;color: #00FF00">
                                    <span><b>Đã toán thành công</b></span>
                                </td>
                            </c:if>
                            <c:if test = "${lpro.orderStatus == '0'}">
                                <td colspan="3" style="text-align: center;color: red">
                                    <span><b>Đã huỷ đơn</b></span>
                                </td>
                            </c:if>    
                           
                            <input name="odstatus" type="hidden" value="${lpro.orderStatus}" />
                            <input name="orderId" type="hidden" value="${lpro.orderId}" />
                            </tr>
                        </c:forEach>
